...
const Project = async ({ params }: Props) => {
  ...
  console.log("THIS IS PROEJCT", project);
  ...
  return (
    <div>
      <header className="grid grid-cols-1 md:grid-cols-2 items-center  my-5">
        <h1 className="text-3xl drop-shadow-md text-center custom-gradient bg-clip-text text-transparent">
          {project.name}
        </h1>
      </header>
      {/* Content */}
      <div className="text-lg text-gray-700 mt-5 grid md:grid-cols-2 grid-cols-1 px-2">
        ...
        <div className="px-5 whitespace-break-spaces">
          <h1 className="text-center text-2xl custom-gradient bg-clip-text text-transparent md:col-start-2  font-semibold my-auto mt-2 mb-2">
            Summary
          </h1>
          <div>
      ...
      <div className="grid my-10 gap-12 md:grid-cols-4 px-5">
        {project.listDetails &&
          project.listDetails.map((listItem) => {
            return (
              <div
                key={listItem.name}
                className="min-w-full flex flex-col justify-items-center"
              >
                <h1 className="text-3xl text-center custom-gradient bg-clip-text text-transparent">
                  {listItem.name}
                </h1>
                <hr />
                <ul className="list-disc marker:text-pink-400 mx-auto">
                  {listItem.listContent.map((bullet, index) => (
                    <li key={`${listItem.name}` + index}>{bullet}</li>
                  ))}
                </ul>
              </div>
            );
